import { useState } from 'react';
import { Title, Input, Selector, Paragraph } from './components';

const BASE_INFUSION_TIME = {
  '1/10': [10, 10, 15, 20, 30, 45, 60, 120, 180, 300, 480],
  '1/15': [10, 10, 15, 20, 30, 45, 60, 120, 180, 300, 480],
  '1/30': [20, 20, 30, 45, 60, 120, 180, 300, 480],
  '1/50': [120, 180, 300, 480],
  '1/100': [300, 480],
};

const CONCENTRATIONS = [
  { id: 10, label: '1/10' },
  { id: 15, label: '1/15' },
  { id: 30, label: '1/30' },
  { id: 50, label: '1/50' },
  { id: 100, label: '1/100' },
];

function App() {
  const [concentration, setConcentration] = useState(10);
  const [size, setSize] = useState(100);
  console.log(concentration, size);
  return (
    <main className="flex flex-col justify-center items-center h-screen w-screen bg-white dark:bg-slate-800">
      <Title>Tea Site</Title>
      <form className="flex flex-col justify-center items-center mt-10 max-w-screen-md">
        <Paragraph className='text-center mb-10'>
          Note that the real amount of tea and the infusion time can vary
          depending on the type of tea and the quality of the same. Please experiment yourself.
        </Paragraph>
        <Selector
          labelChildren="Concentration"
          options={CONCENTRATIONS}
          value={concentration}
          onChange={(e) => setConcentration(Number(e.target.value))}
        />
        <Input
          labelChildren="Size of vessel in ml"
          value={size}
          onChange={(e) => setSize(Number(e.target.value))}
          type="number"
        />
        <Paragraph>
          The amount of tea you need is:{' '}
          <span className="font-bold">{Math.ceil(size / concentration)} g</span>
        </Paragraph>
        <Paragraph>
          Expected infusion time:{' '}
          <span className="font-bold">
            {BASE_INFUSION_TIME[
              `1/${concentration}` as keyof typeof BASE_INFUSION_TIME
            ].join(', ')}{' '}
            seconds
          </span>
        </Paragraph>
      </form>
    </main>
  );
}

export default App;
